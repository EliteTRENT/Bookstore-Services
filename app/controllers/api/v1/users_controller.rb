class Api::V1::UsersController < ApplicationController
  ...
  def login
    ...
    if result[:success]
      render json: { message: result[:message], token: result[:token], user_id: result[:user_id], user_name: result[:user_name], email: result[:email], mobile_number: result[:mobile_number]  }, status: :ok
    else
      render json: { errors: result[:error] }, status: :unprocessable_entity
    end
  end
